...
class curation_blink extends CMS_Controller {

	function __construct() {
		parent::__construct();
		$this->load->model('Curation_model_blink');
	}
    ...
    public function getHomeData() {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_blink->get_homedata_serverside($_POST) );
        } else {
            $this->access_denied();
        }
    }
    ...
    public function getHomeData_archived() {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_blink->get_homedata_archived($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function reorderhomedata() {
        if ($this->userinfo['permissions']->curation_update) {
            $response = $this->Curation_model_blink->update_sortorder_home($_POST);
            echo $response;
    ...
    public function changeStatus() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_blink->update_status_by_platform($_POST);
    ...
    public function archiveItem() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_blink->archive_item($_POST['catcode']);
    ...
    public function unarchiveItem() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_blink->unarchive_item($_POST['catcode']);
    ...
    public function getBooklist($value='') {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_blink->get_booklist_serverside($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function createSection($value='') {
        if ($this->userinfo['permissions']->curation_create) {
            echo $this->Curation_model_blink->create_new_section($_POST);
        } else {
            echo 403;
        }
    }
    ...
    public function getSectionItems($value='') {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_blink->get_section_items_by_catcode($_POST['catcode']) );
        } else {
            echo 403;
        }
    }
    ...
    public function updateSectionSortorder($value='') {
        if ($this->userinfo['permissions']->curation_update) {
            echo json_encode( $this->Curation_model_blink->update_section_items_sortorder($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function removeSectionItem($value='') {
        if ($this->userinfo['permissions']->curation_delete) {
            echo json_encode( $this->Curation_model_blink->remove_section_item($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function addSectionItem($value='') {
        if ($this->userinfo['permissions']->curation_create) {
            echo json_encode( $this->Curation_model_blink->add_section_item($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function editSectionInfo($value='') {
        if ($this->userinfo['permissions']->curation_update) {
            echo json_encode( $this->Curation_model_blink->update_section_info($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function get_banner_items() {
        if ($this->userinfo['permissions']->banner_view) {
            echo json_encode( $this->Curation_model_blink->get_banner_items($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function add_banner_item() {
        $path = "/var/www/html/ebswap/banner_th/blink";
        if(isset($_FILES["file_banner_web"]["name"]) && isset($_FILES["file_banner_mob"]["name"])) {
            ...
            if ($this->upload->do_upload('file_banner_web') && $this->upload->do_upload('file_banner_mob')) {
                $db_insert_result = $this->Curation_model_blink->add_banner_item($_POST);
    ...
    public function change_banner_item_status() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->change_banner_item_status($_POST['id'], $_POST['status']);
            echo $response;
    ...
    public function delete_banner_item() {
        if ($this->userinfo['permissions']->banner_delete) {
            $response = $this->Curation_model_blink->delete_banner_item($_POST['id']);
            echo $response;
    ...
    public function update_banner_item() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->update_banner_item($_POST);
            echo $response;
    ...
    public function reorder_banner() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->update_sortorder_banner($_POST);
            echo $response;
    ...
    public function get_promo_items() {
        if ($this->userinfo['permissions']->banner_view) {
            echo json_encode( $this->Curation_model_blink->get_promo_items($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function add_promo_item() {
        $path = "/var/www/html/ebswap/promo_th/blink";
        if(isset($_FILES["file_promo"]["name"])) {
            ...
            if ($this->upload->do_upload('file_promo')) {
                $db_insert_result = $this->Curation_model_blink->add_promo_item($_POST);
                echo $db_insert_result;
                // echo "1";
    ...
    public function change_promo_item_status() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->change_promo_item_status($_POST['id'], $_POST['status']);
            echo $response;
    ...
    public function delete_promo_item() {
        if ($this->userinfo['permissions']->banner_delete) {
            $response = $this->Curation_model_blink->delete_promo_item($_POST['id']);
            echo $response;
    ...
    public function update_promo_item() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->update_promo_item($_POST);
            echo $response;
    ...
    public function reorder_promo() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_blink->update_sortorder_promo($_POST);
            echo $response;
