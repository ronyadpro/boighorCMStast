...
class curation_airtel extends CMS_Controller {

	function __construct() {
		parent::__construct();
		$this->load->model('Curation_model_airtel');
	}
    ...
    public function getHomeData() {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_airtel->get_homedata_serverside($_POST) );
        } else {
            $this->access_denied();
        }
    }
    ...
    public function getHomeData_archived() {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_airtel->get_homedata_archived($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function reorderhomedata() {
        if ($this->userinfo['permissions']->curation_update) {
            $response = $this->Curation_model_airtel->update_sortorder_home($_POST);
            echo $response;
    ...
    public function changeStatus() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_airtel->update_status_by_platform($_POST);
    ...
    public function archiveItem() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_airtel->archive_item($_POST['catcode']);
    ...
    public function unarchiveItem() {
        if ($this->userinfo['permissions']->curation_update) {
            $resp = $this->Curation_model_airtel->unarchive_item($_POST['catcode']);
    ...
    public function getBooklist($value='') {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_airtel->get_booklist_serverside($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function createSection($value='') {
        if ($this->userinfo['permissions']->curation_create) {
            echo $this->Curation_model_airtel->create_new_section($_POST);
        } else {
            echo 403;
        }
    }
    ...
    public function getSectionItems($value='') {
        if ($this->userinfo['permissions']->curation_view) {
            echo json_encode( $this->Curation_model_airtel->get_section_items_by_catcode($_POST['catcode']) );
        } else {
            echo 403;
        }
    }
    ...
    public function updateSectionSortorder($value='') {
        if ($this->userinfo['permissions']->curation_update) {
            echo json_encode( $this->Curation_model_airtel->update_section_items_sortorder($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function removeSectionItem($value='') {
        if ($this->userinfo['permissions']->curation_delete) {
            echo json_encode( $this->Curation_model_airtel->remove_section_item($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function addSectionItem($value='') {
        if ($this->userinfo['permissions']->curation_create) {
            echo json_encode( $this->Curation_model_airtel->add_section_item($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function editSectionInfo($value='') {
        if ($this->userinfo['permissions']->curation_update) {
            echo json_encode( $this->Curation_model_airtel->update_section_info($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function banner() {
        if ($this->userinfo['permissions']->banner_view) {
			...
			$data['title'] = 'Banner-Curation BanglaLink';
    ...
    public function get_banner_items() {
        if ($this->userinfo['permissions']->banner_view) {
            echo json_encode( $this->Curation_model_airtel->get_banner_items($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function add_banner_item() {
        $path = "/var/www/html/ebswap/banner_th/airtel";
        if(isset($_FILES["file_banner_web"]["name"]) && isset($_FILES["file_banner_mob"]["name"])) {
            ...
            if ($this->upload->do_upload('file_banner_web') && $this->upload->do_upload('file_banner_mob')) {
                $db_insert_result = $this->Curation_model_airtel->add_banner_item($_POST);
    ...
    public function change_banner_item_status() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->change_banner_item_status($_POST['id'], $_POST['status']);
            echo $response;
    ...
    public function delete_banner_item() {
        if ($this->userinfo['permissions']->banner_delete) {
            $response = $this->Curation_model_airtel->delete_banner_item($_POST['id']);
            echo $response;
    ...
    public function update_banner_item() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->update_banner_item($_POST);
            echo $response;
    ...
    public function reorder_banner() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->update_sortorder_banner($_POST);
            echo $response;
    ...
    public function get_promo_items() {
        if ($this->userinfo['permissions']->banner_view) {
            echo json_encode( $this->Curation_model_airtel->get_promo_items($_POST) );
        } else {
            echo 403;
        }
    }
    ...
    public function add_promo_item() {
        $path = "/var/www/html/ebswap/promo_th/airtel";
        if(isset($_FILES["file_promo"]["name"])) {
            ...
            if ($this->upload->do_upload('file_promo')) {
                $db_insert_result = $this->Curation_model_airtel->add_promo_item($_POST);
                echo $db_insert_result;
                // echo "1";
    ...
    public function change_promo_item_status() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->change_promo_item_status($_POST['id'], $_POST['status']);
            echo $response;
    ...
    public function delete_promo_item() {
        if ($this->userinfo['permissions']->banner_delete) {
            $response = $this->Curation_model_airtel->delete_promo_item($_POST['id']);
            echo $response;
    ...
    public function update_promo_item() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->update_promo_item($_POST);
            echo $response;
    ...
    public function reorder_promo() {
        if ($this->userinfo['permissions']->banner_update) {
            $response = $this->Curation_model_airtel->update_sortorder_promo($_POST);
            echo $response;
