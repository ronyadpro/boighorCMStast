...
$(document).ready(function() {
    ...
    $('#DCEcheck').change(function() {
        if($(this).is(":checked")) {
            $(this).attr("checked", true);
            $("#capblock").show();
            $( "#mindiscount" ).prop( "disabled", false );

        }else {
            $("#capblock").hide();
            $( "#mindiscount" ).prop( "disabled", true );
        }
    });

    $('#UCEcheck').change(function() {
        if($(this).is(":checked")) {
            $(this).attr("checked", true);
            $("#capblock").show();
            $( "#txtusagelimit" ).prop( "disabled", false );

        }else {
            $("#capblock").hide();
            $( "#txtusagelimit" ).prop( "disabled", true );
        }
    });

    $('#MBCEcheck').change(function() {
        if($(this).is(":checked")) {
            $(this).attr("checked", true);
            $("#capblock").show();
            $( "#txtcartsize" ).prop( "disabled", false );

        }else {
            $("#capblock").hide();
            $( "#txtcartsize" ).prop( "disabled", true );
        }
    });
...
$('#frm_coupon_create').submit(function(e) {
    e.preventDefault();
    ...
    Swal.fire({
        title: 'Are You Sure?',
        text: 'You are about to Create a New Promotion',
        type: 'question',
        ...
        if (result.value) {
            ...
            form_data.append('DCEcheck', $('input[name=DCEcheck]').val());
            form_data.append('MBCEcheck', $('input[name=MBCEcheck]').val());
            ...
            $.ajax({
                ...
                url: "<?= base_url() ?>coupon/create_coupon",
                method: "POST",
                data: form_data,
                contentType: false,
                processData: false,
                success:function(data) {
                    console.log(data);
                    if (data == 1) {
                        Swal.fire("Promotion Created Successfully", "", "success").then((result) => {
                            location.reload();
                        });
                    } else if (data==403) {
