        ...
        <div class="row mb-4">
            <div class="col-sm-6">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Boigho Global</li>
                </ol>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content bg-light">
            ...
            <form id="frm_add_price_bdt" method="post">
                <input type="hidden" name="servicename" value="boighorglobal">
                <div class="modal-body">
                    <div class="row justify-content-center mt-2">
                        <div class="col-12">
                            <label for="">bookprice</label>
                            <input required type="text" class="form-control" name="bookprice" pattern="^[0-9]{1,4}(.[0-9]{2})?$" id="txt_add_bookprice_bdt" placeholder="15">
                            <!-- <label for="">price_bn</label> -->
                            <input required type="hidden" class="form-control" name="price_bn" pattern="^[০-৯]{1,4}(.[০-৯]{2})?$" id="txt_add_price_bn_bdt">
                            <!-- <label for="">bookprice_en</label> -->
                            <input required type="hidden" class="form-control" name="bookprice_en" pattern="^৳[0-9]{1,4}(.[0-9]{2})?$" id="txt_add_bookprice_en_bdt">
                            <label for="">bookprice_bn</label>
                            <input required type="text" class="form-control" name="bookprice_bn" pattern="^৳[০-৯]{1,4}(.[০-৯]{2})?$" id="txt_add_bookprice_bn_bdt" placeholder="৳১৫">
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-body">
                    <div class="row justify-content-center mt-2">
                        <div class="col-12">
                            ...
                            <!-- <label for="">price_bn</label> -->
                            <input required type="hidden" class="form-control" name="price_bn" pattern="^[০-৯]{1,4}(.[০-৯]{2})?$" id="txt_edit_price_bn_bdt">
                            <!-- <label for="">bookprice_en</label> -->
                            <input required type="hidden" class="form-control" name="bookprice_en" pattern="^৳[0-9]{1,4}(.[0-9]{2})?$" id="txt_edit_bookprice_en_bdt">
                            <label for="">bookprice_bn</label>
...
$('#navlink_pricing_global').addClass('active');
...
$('#frm_edit_price_bdt').submit(function(evnt) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= base_url('price/edit_pricing_global_bdt') ?>",
                type: "POST",
                data: form_data,
                contentType: false,
                processData: false,
...
$('#frm_edit_price_usd').submit(function(evnt) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= base_url('price/edit_pricing_global_usd') ?>",
                type: "POST",
                data: form_data,
                contentType: false,
                processData: false,
...
for (var pair of form_data.entries()) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= base_url('price/add_pricing_global_bdt') ?>",
                type: "POST",
                data: form_data,
                contentType: false,
                processData: false,
...
$('#frm_add_price_usd').submit(function(evnt) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= base_url('price/add_pricing_global_usd') ?>",
                type: "POST",
                data: form_data,
                contentType: false,
                processData: false,
...
$('#txt_add_bookprice_bdt').keyup(function(evnt) {
    $('#txt_add_price_bn_bdt').val(eng_to_bng($('#txt_add_bookprice_bdt').val()))
    $('#txt_add_bookprice_en_bdt').val($('#txt_add_bookprice_bdt').val()+'.00')
    $('#txt_add_bookprice_bn_bdt').val('৳'+eng_to_bng($('#txt_add_bookprice_bdt').val()))
})
$('#txt_edit_bookprice_bdt').keyup(function(evnt) {
    $('#txt_edit_price_bn_bdt').val(eng_to_bng($('#txt_edit_bookprice_bdt').val()))
    $('#txt_edit_bookprice_en_bdt').val($('#txt_edit_bookprice_bdt').val()+'.00')
    $('#txt_edit_bookprice_bn_bdt').val('৳'+eng_to_bng($('#txt_edit_bookprice_bdt').val()))
})
</script>
