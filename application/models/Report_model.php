...
class Report_model extends CI_Model {
    ...
    public function get_sales_report($post) {
        ...
        if ($post['campaign']) $this->db->where('promocode',$post['campaign']);
        if ($post['paymenttype']) $this->db->where('paymenttype',$post['paymenttype']);
        $this->db->select('*');
        $this->db->select('(SELECT fullname FROM tblBoiMelaSubscriptionDetails WHERE msisdn = tblOrderDetails.userid LIMIT 1) AS name');
    ...
    public function get_order_logs($post) {
        $data['start'] = 0;//$post['start'];
        $data['draw'] = $post['draw'];
        $order_col = $post['columns'][$post['order'][0]['column']]['data'];
        $order_dir = $post['order'][0]['dir'];
        ...
        $this->db->select('*');
        $this->db->select('(SELECT fullname FROM tblBoiMelaSubscriptionDetails WHERE msisdn = tblOrderDetails.userid LIMIT 1) AS name');
        $data['data'] = $this->db->where('DATE(created)',$post['date'])->order_by($order_col, $order_dir)->limit($post['length'], $post['start'])->get('tblOrderDetails')->result();
        $data['recordsFiltered'] = $this->db->where('DATE(created)',$post['date'])->count_all_results('tblOrderDetails');
        $data['recordsTotal'] = $this->db->count_all_results('tblOrderDetails');
    ...
    public function get_revenue_report($post) {
        ...
        if ($post['isexclusive']!=='') $this->db->where('boighor.tblBooks.isexclusive',$post['isexclusive']);
        ...
        $this->db->join('boighor.tblBooks','boighor.tblBooks.bookcode = boighor.tblOrderDetails.bookcode');
        $this->db->join('boighor.tblPaymentNotification','boighor.tblPaymentNotification.orderId = boighor.tblOrderDetails.orderid', 'inner');
    ...
    public function get_daily_new_user($duration=30) {
        // $this->db->cache_on();
        $data = $this->db->query(
            'SELECT COUNT(u.msisdn) AS count, DATE_FORMAT(b.Days, "%b-%e") AS date
            FROM (SELECT a.Days
                FROM (
                    SELECT curdate() - INTERVAL (a.a + (10 * b.a) + (100 * c.a)) DAY AS Days
                    FROM       (SELECT 0 AS a UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS a
            ...
            WHERE a.Days >= curdate() - INTERVAL '.$duration.' DAY) b

            LEFT JOIN boighor.tblBoiMelaSubscriptionDetails AS u
            ON DATE(signupdate) = b.Days
    ...
    public function get_top_author_pie_chart_data($duration=7) {
        ...
        $this->db->where("DATE(timeofentry) > curdate() - INTERVAL $duration DAY");
    ...
    function getFreeBookDownloadList(){
        ...
        $query = $this->db->query('SELECT b.Days AS edate,  COUNT(contentid) AS enumbers
            ...
            LEFT JOIN (
                ...
                LEFT JOIN boighor.tblBookPriceTagging AS p ON p.bookcode = c.contentid
    ...
    public function getFreeBookDownloadListForDate($date) {
        $query = $this->db->query(
            ...
            JOIN boighor.tblBookPriceTagging AS p ON p.bookcode = c.contentid
            JOIN boighor.tblBooks AS b ON b.bookcode = c.contentid
    ...
    public function get_mou_report($post) {
        ...
        $this->db->select('boighor.tblContentPlayLog.msisdn AS msisdn, boighor.tblContentPlayLog.adbid AS adbid, boighor.tblBookAudios.bookcode AS bookcode, boighor.tblBookAudios.title AS title, boighor.tblBookAudios.title_bn AS title_bn, boighor.tblBooks.bookname AS bookname, boighor.tblBooks.writercode AS writercode, boighor.tblBooks.writer AS writer, boighor.tblBooks.writer_bn AS writer_bn, boighor.tblBookAudios.filelength AS filelength, SUM(LEAST(playtime,playposition)) AS total_usage_in_seconds, COUNT(*) AS hitcount');
